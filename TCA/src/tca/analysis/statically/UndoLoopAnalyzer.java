...
public class UndoLoopAnalyzer {
	...
	public static void undoLoopAnalyzer() {
		...
		Iterator<GraphElement> itr = universe().nodesTaggedWithAny(XCSG.ControlFlow_Node).eval().nodes().iterator(); 
		while (itr.hasNext()){  
		...
		Iterator<GraphElement> itrE = universe().edgesTaggedWithAny(XCSG.ControlFlow_Edge, XCSG.ExceptionalControlFlow_Edge).eval().edges().iterator();
		while (itrE.hasNext()) {  
